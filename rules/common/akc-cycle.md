...
**A model generation change triggers the downward audit too**, and it is the one case where a surviving rule is worse than no rule. Over-constraints written for a weaker model — blanket prohibitions, exhaustive procedures, repeated emphasis, enumerated examples — have no native duplicate; what expired is their premise. The behaviour they prevented no longer occurs, but the instruction still has to be read and reconciled against the model's own judgement, so the rule turns from idle to actively costly. Usage telemetry will not catch this: an always-loaded rule reports full usage precisely while its firing is the cost. Re-audit the rules layer on each model generation, treat the vendor's own prompt-engineering guidance as admissible evidence, and keep only the gotchas and opinions the model cannot derive.
